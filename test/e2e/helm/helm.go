...
var _ = Describe("HelmRelease", func() {
	...
	BeforeEach(func() {
		...
		k8sCache := informer.NewInformer(framework.GetK8sClient(), framework.GetK8sReleaseConfigClient(), nil,0, stopChan)
	...
	Describe("test install release", func() {
		...
		It("test release plugin", func() {
			...
			manifest := strings.Replace("\n---\napiVersion: v1\nkind: Service\nmetadata:\n  labels:\n    app: tomcat\n    chart: tomcat-0.2.0\n    heritage: Helm\n    release: tomcat-test\n  name: tomcat-test\n  namespace: helmreleasetest-b2g2l\nspec:\n  ports:\n  - name: http\n    port: 80\n    protocol: TCP\n    targetPort: 8080\n  selector:\n    app: tomcat\n    release: tomcat-test\n  type: NodePort\n\n---\napiVersion: apps/v1\nkind: Deployment\nmetadata:\n  creationTimestamp: null\n  labels:\n    app: tomcat\n    chart: tomcat-0.2.0\n    heritage: Helm\n    release: tomcat-test\n  name: tomcat-test\n  namespace: helmreleasetest-b2g2l\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: tomcat\n      release: tomcat-test\n  strategy: {}\n  template:\n    metadata:\n      creationTimestamp: null\n      labels:\n        app: tomcat\n        release: tomcat-test\n        test_key: test_value\n    spec:\n      containers:\n      - image: tomcat:7.0\n        imagePullPolicy: Always\n        livenessProbe:\n          httpGet:\n            path: /sample\n            port: 8080\n          initialDelaySeconds: 60\n          periodSeconds: 30\n        name: tomcat\n        ports:\n        - containerPort: 8080\n          hostPort: 8009\n        readinessProbe:\n          failureThreshold: 6\n          httpGet:\n            path: /sample\n            port: 8080\n          initialDelaySeconds: 60\n          periodSeconds: 30\n        resources:\n          limits:\n            cpu: 200m\n            memory: 200Mi\n          requests:\n            cpu: 100m\n            memory: 100Mi\n        volumeMounts:\n        - mountPath: /usr/local/tomcat/webapps\n          name: app-volume\n      initContainers:\n      - command:\n        - sh\n        - -c\n        - cp /*.war /app\n        image: ananwaresystems/webarchive:1.0\n        imagePullPolicy: Always\n        name: war\n        resources: {}\n        volumeMounts:\n        - mountPath: /app\n          name: app-volume\n      volumes:\n      - emptyDir: {}\n        name: app-volume\nstatus: {}\n\n---\napiVersion: apiextensions.transwarp.io/v1beta1\nkind: ReleaseConfig\nmetadata:\n  creationTimestamp: null\n  name: tomcat-test\n  namespace: helmreleasetest-b2g2l\nspec:\n  chartAppVersion: \"7\"\n  chartImage: \"\"\n  chartName: tomcat\n  chartVersion: 0.2.0\n  configValues: {}\n  dependencies: null\n  dependenciesConfigValues: {}\n  outputConfig: {}\n  repo: \"\"\nstatus: {}\n",
				"helmreleasetest-b2g2l", namespace, -1)
			Expect(releaseCache.Manifest).To(Equal(manifest))
		...
		It("test release pause", func() {
			...
			paused = false
			releaseInfo := &release.ReleaseInfoV2{}
			releaseInfo.Plugins = []*release.ReleasePlugin{
				{
					Version: "1.0",
					Name:    plugins.PauseReleasePluginName,
				},
