...
func NewGREx(cnf *config.Config, addrs []string, db int, otherOptions *common.OtherGoRedisOptions, retryFunc func(chan int)) iface.Broker {
	...
	if otherOptions != nil {
		if otherOptions.MaxRetries != 0 {
			ropt.MaxRetries = otherOptions.MaxRetries
		}
		ropt.MinRetryBackoff = otherOptions.MinRetryBackoff
		ropt.MaxRetryBackoff = otherOptions.MaxRetryBackoff
	}
