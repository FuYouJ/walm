...
func NewWalmPluginManager(kubeClient kube.Interface, r *release.Release, log func(string, ...interface{})) (manager *WalmPluginManager) {
...
func (manager *WalmPluginManager) ExecPlugins(runnerType RunnerType) error {
	...
	if runnerType == Pre_Install {
		resources, err := manager.context.KubeClient.Build(bytes.NewBufferString(manager.context.R.Manifest))
		if err != nil {
