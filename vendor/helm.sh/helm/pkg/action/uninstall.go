...
func (u *Uninstall) Run(name string) (*release.UninstallReleaseResponse, error) {
	...
	if len(errs) > 0 {
		return res, errors.Errorf("delete release failed with %d error(s): %s", len(errs), joinErrors(errs))
	}
	res.Info = kept

	if !u.DisableHooks {
		if err := u.cfg.execHook(rel, release.HookPostDelete, u.Timeout); err != nil {
			u.cfg.Log("Failed to exec hook: %s", err.Error())
			return res, err
		}
	}
	...
	if !u.KeepHistory {
		...
		if err != nil {
			u.cfg.Log("Failed to purge releases: %s", err.Error())
